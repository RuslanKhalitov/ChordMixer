...
from .utils.preprocess_genbank import PrepareGenbank
from .utils.collators import collate_batch_horizontal, collate_batch_pad

class GenbankDataModule(pl.LightningDataModule):
    def __init__(
        self,
        data_dir,
        taskname,
    ...
    def process_dataset(self):
        ...
        file = self.data_dir / f"{self.taskname}_train.csv"
        if not file.exists():
            PrepareGenbank(self.data_dir, self.taskname)
            
        # Read train and test csv
        dataset = load_dataset(
            "csv",
            data_files={
                "train": str(self.data_dir / f"{self.taskname}_train.csv"),
                "test": str(self.data_dir / f"{self.taskname}_test.csv")
            },
            keep_in_memory=True,
        )
